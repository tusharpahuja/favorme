...
if(isset($_POST['update']))
{
	...
	if(!empty($category) && !empty($ask_favor))
	{
		if($category=="foods")
		{	
			$query="INSERT INTO foods(user_id,favor,time,date) VALUES($current_user_id,'$ask_favor','$time','$date')";
			$result=mysqli_query($connection,$query);
			if($result)
				header("location: timeline-foods.php");
			else
...
<body>
	<form action="timeline-foods.php" method="POST">
		<div id="container" class="container">
			...
			<div class="main">
				...
				<ul class="cbp_tmtimeline">
					<li>
						<?php 
						$query="SELECT * FROM foods ORDER BY favor_id DESC";
						$result=mysqli_query($connection,$query);
						if($result)
						{
							while($row=mysqli_fetch_assoc($result))
							{
								...
								<div class="cbp_tmlabel">
									...
									<p>
										<div class="baseline">
											<?php 
											$query_comments="SELECT * FROM foods_comments WHERE favor_id=$favor_id ORDER BY comment_id ASC";
...
if(isset($_POST['comment']))
{
	...
	echo "$comments";
	$query_insert_comment="INSERT INTO foods_comments(user_id,favor_id,comment,date,time) VALUES($current_user_id,$idButton,'$comments','$date','$time')";
	...
	if($result_insert_comment)
	{
		...
		?>
		<script type="text/javascript">
			window.open ('timeline-foods.php','_self',false);
		</script>
		<?php
	}
	else
